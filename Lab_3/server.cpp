...
Aws::String bucketName = "user-status";
Aws::String objectName = "1.txt"; // [CONFIG] x = 1, 2, 3
string serverId = "1";  // [CONFIG] x = 1, 2, 3
...
bool AwsDoc::S3::PutObjectBuffer(const Aws::String &bucketName,
    ...
    *inputData << objectContent.c_str();
    ...
    if (!outcome.IsSuccess())
        cerr << "Error: PutObjectBuffer: " <<
                  outcome.GetError().GetMessage() << endl;
    else
        cout << "Successfully put: " << objectName << endl;

    return outcome.IsSuccess();
}

string AwsDoc::S3::GetObject(const Aws::String &objectKey,
    ...
    if (!outcome.IsSuccess()){
        ...
        cout << "Successfully retrieved: " << objectKey << endl;
    }
...
void AWS_update_status(){
    Aws::SDKOptions options;
    Aws::InitAPI(options);
    {
        int cnt = count_online_user();
        string content = "Server" + serverId + ": " + to_string(cnt) + "\n";

        Aws::Client::ClientConfiguration clientConfig;
        clientConfig.region = "us-east-1";

        AwsDoc::S3::PutObjectBuffer(bucketName, objectName, content, clientConfig);
    }
...
string statusHandler(){
    ...
    Aws::InitAPI(options);
    {
        const Aws::String object1 = "1.txt";
        const Aws::String object2 = "2.txt";
        const Aws::String object3 = "3.txt";

        Aws::Client::ClientConfiguration clientConfig;
        clientConfig.region = "us-east-1";

        result += AwsDoc::S3::GetObject(object1, bucketName, clientConfig);
        result += AwsDoc::S3::GetObject(object2, bucketName, clientConfig);
        result += AwsDoc::S3::GetObject(object3, bucketName, clientConfig);
    }
    Aws::ShutdownAPI(options);
...
int main(int argc, char* argv[])
{
    ...
    AWS_update_status(); // init AWS status
